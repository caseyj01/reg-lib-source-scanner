// Known regulatory authorities monitored by Vixio
// 2078 entries
export const KNOWN_AUTHORITIES = [
  'Abu Dhabi Global Market',
  'Abu Dhabi Office of Data Protection',
  'Access to Cash Review',
  'Administrative Council for Economic Defense - Brazil',
  'Advertising Regulatory Board',
  'Advertising Standards Authority',
  'Advertising Standards Authority for Ireland',
  'Advertising Standards Authority of Singapore',
  'Advertising Standards Council of India',
  'Agence de Régulation des Télécommunications et des Postes',
  'Agence Française Anticorruption',
  'Agence Nationale des Infrastructures Numériques et des Fréquences',
  'Agence Sénégalaise de la Promotion de l\'Investissement et des Grands Travaux',
  'Agencia Espanola Proteccion Datos',
  'Agency Against Corruption',
  'Agency for Digital Italy',
  'Agency for Personal Data Protection',
  'Agency for Protection of Competition',
  'Alabama Attorney General',
  'Alabama Legislature',
  'Alabama Securities Commission',
  'Alabama State Banking Department',
  'Alaska Attorney General',
  'Alaska Consumer Protection Unit',
  'Alaska Department of Commerce, Community and Economic Development',
  'Alaska Department of Law',
  'Alaska Division of Banking and Securities',
  'Alaska Governor',
  'Alaska Legislature',
  'Alaska State Legislature',
  'Albania Financial Intelligence Agency',
  'Albania National Cyber Security Authority',
  'Albanian Competition Authority',
  'Albanian Financial Intelligence Agency',
  'Albanian Government',
  'Albanian Government Council of Ministers',
  'Albanian Parliament',
  'Alliance for Financial Inclusion',
  'American Bankers Association',
  'American Samoa Department of Commerce',
  'Angola Data Protection Agency',
  'Angola Financial Intelligence Unit',
  'Angola Ministry of Telecommunications and Information Technology',
  'Angola National Institute for Consumer Protection',
  'Anti-Money Laundering and Counter-Terrorism Financing Office',
  'Anti-Money Laundering Authority (AMLA) - EU',
  'Anti-Money Laundering Division (Executive Yuan)',
  'Antimonopoly Office of the Slovak Republic',
  'Arbitration Commission',
  'Argentina Ministry of Economy',
  'Argentine Bankers Association',
  'Argentine Government',
  'Argentinian Agency of Access to Public Information',
  'Arizona Attorney General',
  'Arizona Commerce Authority',
  'Arizona Corporation Commission',
  'Arizona Department of Insurance and Financial Institutions',
  'Arizona Governor',
  'Arizona Legislature',
  'Arkansas Attorney General',
  'Arkansas Department of Commerce',
  'Arkansas General Assembly',
  'Arkansas Governor',
  'Arkansas Legislature',
  'Arkansas Securities Department',
  'Arkansas State Bank Department',
  'Armenia Personal Data Protection Agency',
  'Armenia State Commission for the Protection of Economic Competition',
  'ASEAN (Association of Southeast Asian Nations)',
  'Asia/Pacific Group on Money Laundering (APG)',
  'Asian Financial Cooperation Association',
  'Assembly of Brčko District',
  'Assembly of the Republic of North Macedonia',
  'Association Française des Trésoriers d\'Entreprise',
  'Association of Argentine Banks',
  'Association of Banks in Cambodia',
  'Association of Banks in Fiji',
  'Association of Banks of Singapore',
  'Association of Compliance Officers in Ireland',
  'Association of Cryptocurrency Enterprises and Start-up',
  'Association of German Banks',
  'Association of German Public Banks',
  'Association of Local Banks, Savings Banks and Cooperative Banks in Denmark',
  'Association of Payment and Electronic Money Institutions of Turkey',
  'Association of Payment Service Providers',
  'Association of Serbian Banks',
  'Association of Swedish Finance Houses',
  'Atlanta Federal Reserve',
  'Attorney General\'s Office - Brunei',
  'Attorney General\'s Office - Tonga',
  'Attorney-General\'s Department',
  'Australian Association of National Advertisers',
  'Australian Banking Association',
  'Australian Communications and Media Authority',
  'Australian Competition and Consumer Commission',
  'Australian Cyber Security Centre',
  'Australian Department of Home Affairs',
  'Australian Department of Industry, Innovation and Science',
  'Australian Finance Industry Association Limited (AFIA)',
  'Australian Financial Complaints Authority',
  'Australian Government',
  'Australian House of Representatives',
  'Australian Human Rights Commission',
  'Australian Parliament',
  'Australian Payments Council',
  'Australian Payments Network',
  'Australian Prudential Regulation Authority',
  'Australian Retail Credit Association',
  'Australian Securities and Investments Commission',
  'Australian Signals Directorate',
  'Australian Transaction Reports and Analysis Centre',
  'Australian Treasury',
  'Austria Directorate of State Security and Intelligence Service',
  'Austrian Chamber of Commerce (WKO)',
  'Austrian Data Protection Council',
  'Austrian Federal Council',
  'Austrian Federal Criminal Police Office',
  'Austrian Federal Ministry of Finance',
  'Austrian Government',
  'Austrian Parliament',
  'Authority for Anti-Money Laundering and Countering the Financing of Terrorism',
  'Autorità Nazionale Anticorruzione',
  'Autorité de Contrôle Prudentiel et de Résolution',
  'Autorité de la concurrence',
  'Autorité de Régulation des Communications Electroniques, des Postes et de la Distribution de la Presse',
  'Autorité de Régulation des Postes, Télécommunications et Technologies de l\'Information et de la Communication',
  'Autorité de Régulation Professionnelle de la Publicité',
  'Autorité des Marchés Financiers (AMF)',
  'Autorité des Marchés Financiers de l\'Union Monétaire Ouest Africaine',
  'Azerbaijan Competition Authority',
  'Azerbaijan Ministry of Digital Development and Transport',
  'AZLP (Data Protection Agency)',
  'Bahrain Government',
  'Banco de España',
  'Banco de la República Colombia',
  'Banco de México',
  'Banco de Portugal',
  'Bangko Sentral ng Pilipinas',
  'Bangladesh Bank',
  'Bangladesh Competition Commission',
  'Bangladesh Financial Intelligence Unit',
  'Bangladesh Government',
  'Bangladesh Parliament',
  'Bangladesh Securities and Exchange Commission',
  'Bank for Central African States',
  'Bank for International Settlements',
  'Bank Guarantee Fund',
  'Bank Indonesia',
  'Bank Negara Malaysia',
  'Bank of Albania',
  'Bank of Canada',
  'Bank of Central African States',
  'Bank of England',
  'Bank of Finland',
  'Bank of Ghana',
  'Bank of Greece',
  'Bank of Guyana',
  'Bank of Indonesia',
  'Bank of Israel',
  'Bank of Italy',
  'Bank of Japan',
  'Bank of Korea',
  'Bank of Lithuania',
  'Bank of Mauritius',
  'Bank of Namibia',
  'Bank of Papua New Guinea',
  'Bank of Thailand',
  'Bank of the Lao P.D.R',
  'Bank of the Republic of Burundi',
  'Bank of Uganda',
  'Bank of West African States',
  'Bank of Zambia',
  'Bankers Association Of The Republic Of China',
  'Banking Agency of Federation of Bosnia and Herzegovina',
  'Banking Agency of Republika Srpska (ABRS)',
  'Banking and Financial Ombudsman',
  'Banking and Payments Federation Ireland',
  'Banking Association South Africa',
  'Banking Codes and Standards Board of India',
  'Banking Commission of the West African Monetary Union',
  'Banking Finance and Insurance Institute of Nepal',
  'Banking Standards Board',
  'Banking Supervision Department',
  'Banque Centrale de la République de Guinée',
  'Banque de France',
  'Banque du Liban',
  'Basel Committee on Banking Supervision',
  'Belarus Department of Financial Monitoring (Financial Intelligence Unit)',
  'Belarus Ministry of Antimonopoly Regulation and Trade',
  'Belarus Ministry of Communications and Informatization',
  'Belarus State Control Committee',
  'Belarusian Government',
  'Belarusian National Assembly',
  'Belgian Chamber of Representatives',
  'Belgian Competition Authority',
  'Belgian Constitutional Court',
  'Belgian Council of Ministers',
  'Belgian Data Protection Authority',
  'Belgian Federal Government',
  'Belgian Federal Parliament',
  'Belgian Financial Intelligence Processing Unit',
  'Belgian House of Representatives',
  'Belgium Federal Public Service Economy, SMEs, Middle Classes and Energy',
  'Benin Information Systems and Digital Agency',
  'Benin National Financial Information Processing Unit',
  'Benin Regulatory Authority for Electronic Communications and Post Office',
  'Bermuda Monetary Authority',
  'BfDI',
  'Bhutan Ministry of Finance',
  'Bits AS',
  'Board of Governors of the Federal Reserve System',
  'Bosnian Parliament',
  'Brazil Superintendence of Private Insurance',
  'Brazilian Chamber of Deputies',
  'Brazilian Federal Government',
  'Brazilian National Congress',
  'Brazilian Securities and Exchange Commission',
  'British Bankers\' Association (BBA)',
  'British Columbia Consumer Protection Authority',
  'British Columbia Financial Services Authority',
  'British Columbia Securities Commission',
  'Brunei Association of Banks',
  'Brunei Central Bank',
  'Brunei Department of Competition and Consumer Affairs',
  'Bulgaria Competition Commission',
  'Bulgaria Council of Ministers',
  'Bulgaria Ministry of Economy and Industry',
  'Bulgaria State Agency for National Security',
  'Bulgarian Commission for Personal Data Protection',
  'Bulgarian Council of Ministers',
  'Bulgarian Financial Intelligence Directorate',
  'Bulgarian Financial Supervision Commission',
  'Bulgarian Government',
  'Bulgarian Ministry of Interior',
  'Bulgarian National Assembly',
  'Bulgarian National Bank',
  'Bulgarian Parliament',
  'Bundeskartellamt',
  'Bundesrat',
  'Bundestag',
  'Burkina Faso Commission for Information Technology and Freedoms',
  'Burkina Faso Data Protection Authority',
  'Burkina Faso National Order of Chartered Accountants and Certified Accountants',
  'Burkina Faso Regulatory Authority for Electronic Communications and Postal Services',
  'Burundi Capital Markets Regulatory Authority',
  'Burundi Insurance Regulation and Control Agency',
  'Cabinet of Estonia',
  'Cabinet of Iceland',
  'Cabinet of Thailand',
  'Cabinet Secretariat of Indonesia',
  'Cabinet Secretary for the Economy and Finance - Macau',
  'California Attorney General',
  'California Department of Cannabis Control',
  'California Department of Financial Protection and Innovation',
  'California Governor\'s Office',
  'California Privacy Protection Agency',
  'California State Legislature',
  'Cámara de Diputados',
  'Cambodia Financial Intelligence Unit',
  'Cambodia Ministry of Commerce',
  'Cambodian Royal Government',
  'Canadian Bankers Association',
  'Canadian Centre for Cyber Security',
  'Canadian Government',
  'Canadian Investment Regulatory Organization',
  'Canadian Marketing Association',
  'Canadian Parliament',
  'Canadian Radio-television and Telecommunications Commission',
  'Canadian Securities Administrators',
  'Capital Market Authority - Saudi Arabia',
  'Capital Markets Authority - Kenya',
  'Capital Markets Board of Turkey',
  'Cayman Islands Monetary Authority',
  'CCN-CERT Spain',
  'Center for Financial Industry Information Systems',
  'Central African Banking Commission',
  'Central African Financial Market Supervisory Commission',
  'Central African Internet and Security Cell',
  'Central Bank for the Republic of Kosovo (BQK)',
  'Central Bank of Armenia',
  'Central Bank of Bahrain',
  'Central Bank of Bangladesh',
  'Central Bank of Barbados',
  'Central Bank of Bolivia',
  'Central Bank of Bosnia and Herzegovina',
  'Central Bank of Brazil',
  'Central Bank of Chile',
  'Central Bank of Colombia',
  'Central Bank of Comoros',
  'Central Bank of Congo',
  'Central Bank of Costa Rica',
  'Central Bank of Curaçao and Sint Maarten',
  'Central Bank of Cyprus',
  'Central Bank of Djibouti',
  'Central Bank of Ecuador',
  'Central Bank of Egypt',
  'Central Bank of Honduras',
  'Central Bank of Iceland',
  'Central Bank of Ireland',
  'Central Bank of Jordan',
  'Central Bank of Kenya',
  'Central Bank of Lesotho',
  'Central Bank of Luxembourg',
  'Central Bank of Malta',
  'Central Bank of Montenegro',
  'Central Bank of Myanmar',
  'Central Bank of Nicaragua',
  'Central Bank of Nigeria',
  'Central Bank of Oman',
  'Central Bank of Paraguay',
  'Central Bank of Samoa',
  'Central Bank of Seychelles',
  'Central Bank of Solomon Islands',
  'Central Bank of Sri Lanka',
  'Central Bank of Tanzania',
  'Central Bank of the Argentine Republic',
  'Central Bank of the Bahamas',
  'Central Bank of the Dominican Republic',
  'Central Bank of the Republic of Azerbaijan',
  'Central Bank of the Republic of China (Taiwan)',
  'Central Bank of the Republic of Germany',
  'Central Bank of the Republic of Turkey',
  'Central Bank of the Russian Federation',
  'Central Bank of the UAE',
  'Central Bank of Turkmenistan',
  'Central Bank of Uruguay',
  'Central Bank of Uzbekistan',
  'Central Bank of Venezuela',
  'Central Bank of West African States',
  'Central Council for Financial Services Information',
  'Central KYC Records Registry (CKYCR) - India',
  'Central Registry of Securitisation Asset Reconstruction and Security Interest of India',
  'Central Registry of the Republic of North Macedonia',
  'Central Reserve Bank of Peru',
  'Central Vigilance Commission',
  'Centre for Cyber Security Belgium',
  'Centre for Cybersecurity Belgium',
  'Chamber of Deputies of Argentina',
  'Chamber of Deputies of Chile',
  'Chamber of Digital Commerce',
  'Chamber of Senators of Uruguay',
  'Chile National Congress',
  'Chile National Consumer Service',
  'Chile Risk Classification Commission',
  'Chilean Financial Analysis Unit',
  'Chilean Financial Market Commission (CMF)',
  'Chilean Government',
  'Chilean National Congress',
  'China Anti-Money Laundering Monitoring and Analysis Center',
  'China Banking and Insurance Regulatory Commission',
  'China Banking Regulatory Commission',
  'China Cyberspace Administration',
  'China Ministry of Commerce',
  'China Office of Cyberspace Affairs Commission',
  'China Securities Regulatory Commission',
  'China State Administration for Market Regulation',
  'Clearing Corporation of India',
  'Cleveland Federal Reserve',
  'Climate Financial Risk Forum',
  'Colombia Fintech',
  'Colombia Ministry of Information and Communications Technologies',
  'Colombia Unit for Regulatory Projection and Financial Regulation Studies',
  'Colombian Congress',
  'Colombian Financial Information and Analysis Unit (UIAF)',
  'Colombian Government',
  'Colorado Attorney General',
  'Colorado Department of Regulatory Agencies',
  'Colorado Division of Banking',
  'Colorado Division of Securities',
  'Colorado General Assembly',
  'Colorado Governor',
  'Colorado Governor\'s Office of Information Technology',
  'Colorado Legislature',
  'Comisión Arbitral del Convenio Multilateral (Comarb)',
  'Commission Bancaire de l\'Union Monétaire Ouest Africaine',
  'Commission de Surveillance du Secteur Financier',
  'Commission for Personal Data Protection',
  'Commission for the Enrichment of the French Language',
  'Commission Nationale de l\'Informatique et des Libertés',
  'Commissione Nazionale per le Societa e la Borsa',
  'Commissioner for Information of Public Importance and Personal Data Protection',
  'Commissioner of Taxation',
  'Committee for Setting Limits to Exchange Rates',
  'Committee of Advertising Practice (CAP)',
  'Commodity Futures Trading Commission',
  'Common Market for Eastern and Southern Africa',
  'Commonwealth Scientific and Industrial Research Organisation',
  'Comoros International Banking Authority',
  'Companies House',
  'Competition and Consumer Commission of Singapore',
  'Competition Appeal Tribunal',
  'Competition Authority (Concentrations)',
  'Competition Authority of Kenya',
  'Competition Bureau Canada',
  'Competition Commission of India',
  'Competition Commission of Pakistan',
  'Competition Commission of South Africa',
  'Competition Council of the Republic of Lithuania',
  'Conférence Interafricaine des Marchés d\'Assurance',
  'Conference of State Bank Supervisors',
  'Congolese Office of Control',
  'Congress of Paraguay',
  'Congress of the Philippines',
  'Congress of the Republic of Peru',
  'Congress of the Union',
  'Connecticut Attorney General',
  'Connecticut Department of Banking',
  'Connecticut Legislature',
  'Conseil d\'État',
  'Conseil de l\'Ethique Publicitaire',
  'Conseil Paritaire de la Publicité',
  'Consultative Assembly of Saudi Arabia',
  'Consumer Authority - Norway',
  'Consumer Financial Protection Bureau',
  'Consumer Protection British Columbia',
  'Consumer Protection Competition and Fraud Repression Directorate',
  'Cooperation Group for Information Security (SAMFI)',
  'Corrupt Practices Investigation Bureau',
  'Corruption Prevention',
  'Cortes Generales',
  'Costa Rica Data Protection Agency for Residents',
  'Costa Rica Data Protection Authority',
  'Costa Rica National Commission of Ethics and Values',
  'Costa Rican Legal Information System',
  'Council for Financial Activities Control',
  'Council of Europe',
  'Council of Financial Regulators',
  'Council of Ministers of Bosnia and Herzegovina',
  'Council of the District of Columbia',
  'Council of the EU',
  'Court of Justice of the European Union',
  'Croatia Cybersecurity Authority',
  'Croatian Data Protection Authority',
  'Croatian Financial Services Supervision Agency (HANFA)',
  'Croatian Ministry of Public Administration',
  'Croatian National Bank',
  'Croatian Parliament',
  'Curaçao Fair Trade Authority',
  'Curaçao Parliament',
  'Customs and Monopolies Agency (ADM)',
  'Cyber Incident Response Institution',
  'Cybersecurity & Infrastructure Security Agency',
  'Cybersecurity Agency of Singapore',
  'CyberSecurity Malaysia',
  'Cyprus Commission for the Protection of Competition',
  'Cyprus Government',
  'Cyprus Office of the Commissioner for Personal Data Protection',
  'Cyprus Securities and Exchange Commission',
  'Czech Bank Card Association',
  'Czech Chamber of Deputies',
  'Czech Fintech Association',
  'Czech National Bank',
  'Czech Republic Senate',
  'Dallas Federal Reserve',
  'Danish Business Authority',
  'Danish Centre for Cyber Security',
  'Danish Competition and Consumer Authority',
  'Danish Consumer Ombudsman',
  'Danish Data Protection Agency',
  'Danish Director of Public Prosecutions\' Office',
  'Danish Financial Supervisory Authority',
  'Danish Finanstilsynet',
  'Danish Gambling Authority',
  'Danish Government',
  'Danish Parliament',
  'Danmarks Nationalbank',
  'Data and Marketing Association',
  'Data Protection (IDP)',
  'Data Protection Authority of Kosovo (IPA)',
  'Data Security Council of India',
  'Data Standards Body',
  'Datenschutzkonferenz',
  'Delaware Attorney General',
  'Delaware Department of Justice',
  'Delaware Department of State',
  'Delaware Division of Corporations',
  'Delaware General Assembly',
  'Delaware Governor',
  'Delaware Legislature',
  'Department of Banking - Texas',
  'Department of Banking and Securities',
  'Department of Banking Pennsylvania',
  'Department of Commerce, Community, and Economic Development',
  'Department of Finance - Papua New Guinea',
  'Department of Finance - US Virgin Islands',
  'Department of Finance and Treasury - Vanuatu',
  'Department of Finance Canada',
  'Department of Housing and Urban Development (HUD)',
  'Department of Insurance, Securities and Banking',
  'Department of Justice Canada',
  'Department of Justice of Puerto Rico',
  'Department of the Taoiseach',
  'Der Verband österreichischer Banken und Bankiers',
  'Digital Lenders Association of India',
  'Digital Regulation Cooperation Forum (DRCF)',
  'Digital Transformation Agency',
  'Direction Nationale du Contrôle des Marchés Publics',
  'Directorate General of Population and Civil Registration',
  'Directorate of National Consumer Rights Protection',
  'District of Columbia Attorney General',
  'District of Columbia Department of Insurance, Securities and Banking',
  'District of Columbia Legislature',
  'Djibouti Guarantee Fund',
  'Dominican Republic Superintendency of Pensions',
  'Dominican Republic Superintendent of the Stock Market',
  'DR Congo Fund Transfer Regulatory Agency',
  'DR Congo National Financial Intelligence Unit',
  'Dubai Executive Office of Anti-Money Laundering and Counter Terrorism Financing',
  'Dubai Financial Services Authority',
  'Dubai International Financial Centre',
  'Dubai Telecommunications and Digital Government Regulatory Authority (TDRA)',
  'Dubai Virtual Assets Regulatory Authority',
  'Dutch Authority for Consumers and Market',
  'Dutch Authority for the Financial Markets',
  'Dutch Banking Association',
  'Dutch Central Bank',
  'Dutch Cyber Security Council',
  'Dutch Data Protection Authority',
  'Dutch Financial Supervision Office',
  'Dutch Ministry of Foreign Affairs',
  'Dutch National Cyber Security Centre',
  'Dutch Parliament',
  'Dutch Payments Association',
  'Dutch State Treasury Agency',
  'Eastern and Southern Africa Anti-Money Laundering Group',
  'Eastern Caribbean Central Bank',
  'Ecuador Government',
  'Ecuador National Assembly',
  'Ecuador Superintendency of Companies, Securities and Insurance',
  'EEA Joint Committee',
  'Egypt Financial Regulatory Authority',
  'Egyptian Anti-Money Laundering and Terrorist Financing Combating Unit',
  'Egyptian Competition Authority',
  'Egyptian Consumer Protection Agency',
  'Egyptian Court of Cassation',
  'Egyptian Government',
  'Egyptian House of Representatives',
  'El Salvador Central Reserve Bank',
  'El Salvador Consumer Protection Agency',
  'El Salvador Financial Investigation Unit',
  'El Salvador Institute for Access to Public Information',
  'El Salvador Legislative Assembly',
  'El Salvador National Commission of Digital Assets',
  'El Salvador Superintendency of the Financial System',
  'Electronic Communications and Postal Regulatory Authority',
  'Electronic Communications, Postal and Print Media Distribution Regulatory Authority (ARCEP)',
  'Electronic Money Association',
  'Electronic Transactions Association',
  'Electronic Transactions Commission',
  'Electronic Transactions Development Agency',
  'Emerging Payments Association',
  'EMVCo',
  'Enterprise Singapore',
  'Estonia Competition Authority',
  'Estonia Consumer Protection and Technical Surveillance Authority',
  'Estonian Data Protection Inspectorate',
  'Estonian Financial Intelligence Unit',
  'Estonian Financial Supervision Authority',
  'Estonian Government',
  'Estonian Ministry of Justice',
  'Estonian Parliament',
  'Eurasian Group (EAG)',
  'Euro Banking Association',
  'European Association for Secure Transactions',
  'European Association of Payment Service Providers for Merchants',
  'European Banking Authority',
  'European Banking Federation',
  'European Cards Stakeholders Group',
  'European Central Bank',
  'European Commission',
  'European Committee for Standardisation',
  'European Committee of the Regions',
  'European Council',
  'European Court of Auditors',
  'European Credit Sector Associations',
  'European Cyber Security Organisation',
  'European Data Protection Board',
  'European Data Protection Supervisor',
  'European Economic and Social Committee',
  'European Financial Reporting Advisory Group',
  'European Financial Reporting Advisory Group (EFRAG)',
  'European Fund and Asset Management Association',
  'European Insurance and Occupational Pensions Authority',
  'European Parliament',
  'European Payments Council',
  'European Payments Initiative (EPI)',
  'European Securities and Markets Authority',
  'European Systemic Risk Board',
  'European Telecommunications Standards Institute',
  'European Union Agency for Cybersecurity (ENISA)',
  'European Union Agency for Fundamental Rights',
  'European Union Loan Market Association',
  'European Union\'s Judicial Cooperation Unit',
  'Europol',
  'Executive Yuan - Taiwan',
  'Fair Trade Commission',
  'Febelfin',
  'Federal Agency for Financial Market Stabilisation - Germany',
  'Federal Agency on Technical Regulating and Metrology of Russia',
  'Federal Antimonopoly Service of the Russian Federation',
  'Federal Assembly - Sweden',
  'Federal Commissioner for Data Protection and Freedom of Information',
  'Federal Communications Commission',
  'Federal Competition and Consumer Protection Commission',
  'Federal Court of Australia',
  'Federal Criminal Police',
  'Federal Criminal Police Office - Germany',
  'Federal Data Protection and Information Commissioner',
  'Federal Department of Finance',
  'Federal Department of Justice and Police',
  'Federal Deposit Insurance Corporation',
  'Federal Financial Institutions Examination Council',
  'Federal Financial Monitoring Service (Rosfinmonitoring)',
  'Federal Financial Supervisory Authority - BaFin',
  'Federal Ministry of Finance - Germany',
  'Federal Ministry of Justice and Consumer Protection - Germany',
  'Federal Ministry of the Interior, Building and Community - Germany',
  'Federal Office for Information Security - Germany',
  'Federal Office for Protection of the Constitution and Fight Against Terrorism',
  'Federal Office of Justice - Germany',
  'Federal Public Service Finance - Belgium',
  'Federal Register',
  'Federal Reserve',
  'Federal Reserve Bank of Minneapolis',
  'Federal Reserve Bank of San Francisco',
  'Federal Security Service of the Russian Federation (FSB)',
  'Federal Service for Supervision of Communications, Information Technology and Mass Media',
  'Federal Service for Supervision of Consumer Protection and Human Welfare',
  'Federal Telecommunications Institute',
  'Federal Trade Commission',
  'Federal Treasury',
  'Federation Council of the Federal Assembly of the Russian Federation',
  'Federation of Bosnia and Herzegovina Securities Commission',
  'Federation of German Consumer Organisations',
  'Fiji Attorney General',
  'Fiji Consumer Council',
  'Fiji Financial Intelligence Unit',
  'Fijian Competition and Consumer Commission',
  'Finance Commission of Texas',
  'Finance Denmark',
  'Finance Finland',
  'Finance Malta',
  'Finance Norway',
  'Finance Office of the Presidency of Türkiye',
  'Financial Action Task Force',
  'Financial Action Task Force of Latin America',
  'Financial Analytical Unit - Czech Republic',
  'Financial and Monetary Regulation of Ecuador',
  'Financial Arbitration Board',
  'Financial Conduct Authority',
  'Financial Consumer Agency of Canada',
  'Financial Crime Investigation Service (Lithuania)',
  'Financial Crimes Enforcement Network',
  'Financial Crimes Investigation Board',
  'Financial Fraud Action UK',
  'Financial Industry Regulatory Authority',
  'Financial Information and Analysis Unit - Uruguay',
  'Financial Information Unit - Argentina',
  'Financial Intelligence Analysis Unit - Malta',
  'Financial Intelligence Centre - South Africa',
  'Financial Intelligence Unit - India',
  'Financial Intelligence Unit - Luxembourg',
  'Financial Intelligence Unit - Mexico',
  'Financial Intelligence Unit - Portugal',
  'Financial Intelligence Unit - Sri Lanka',
  'Financial Intelligence Unit - The Netherlands',
  'Financial Intelligence Unit of Kosovo',
  'Financial Intelligence Unit of Nepal',
  'Financial Intelligence Unit of North Macedonia (UFR)',
  'Financial Intelligence Unit of Seychelles',
  'Financial Investigation Unit of Curaçao',
  'Financial Market Authority - Austria',
  'Financial Market Authority - Liechtenstein',
  'Financial Market Stability Board - Austria',
  'Financial Markets Authority - New Zealand',
  'Financial Markets Authority of the West African Monetary Union',
  'Financial Monitoring Agency - Kazakhstan',
  'Financial Monitoring Department - Turkmenistan',
  'Financial Monitoring Service',
  'Financial Monitoring Service of the Republic of Azerbaijan',
  'Financial Ombudsman Institution',
  'Financial Ombudsman Service - UK',
  'Financial Regulators Complaints Commissioner',
  'Financial Regulatory Authority - Egypt',
  'Financial Reporting Council',
  'Financial Secretary of the Government of the Hong Kong Special Administrative Region',
  'Financial Sector Commission on Modern Slavery and Human Trafficking',
  'Financial Sector Conduct Authority',
  'Financial Services Agency - Japan',
  'Financial Services and Markets Authority - Belgium',
  'Financial Services and Pensions Ombudsman',
  'Financial Services and the Treasury Bureau',
  'Financial Services Commission - Barbados',
  'Financial Services Commission - South Korea',
  'Financial Services Consumer Panel',
  'Financial Services Regulatory Authority - Abu Dhabi',
  'Financial Services Regulatory Authority - DR Congo',
  'Financial Services Regulatory Authority of Ontario',
  'Financial Services Regulatory Initiatives Forum',
  'Financial Services Sector Coordinating Council',
  'Financial Stability Board',
  'Financial Stability Committee - Germany',
  'Financial Stability Oversight Council',
  'Financial Supervision Authority - Poland',
  'Financial Supervisory Authority',
  'Financial Supervisory Authority - Finland',
  'Financial Supervisory Authority - Romania',
  'Financial Supervisory Authority - Sweden',
  'Financial Supervisory Authority (AMF)',
  'Financial Supervisory Commission',
  'Financial Supervisory Commission - Cook Islands',
  'Financial Supervisory Service',
  'Financial Transactions and Reports Analysis Centre of Canada',
  'Finanspolisen Rikskriminalpolisen',
  'Finnish Competition and Consumer Authority',
  'Finnish Cybersecurity Centre',
  'Finnish Government',
  'Fintech and Payments Association of Ireland',
  'Fintech Association of Hong Kong',
  'Fintech Association of Japan',
  'FinTech Australia',
  'Fintech Finland',
  'Fintech Ireland',
  'Fintech Latvia',
  'Fintech Saudi',
  'FinTechNZ',
  'Florida Attorney General',
  'Florida Department of Financial Services',
  'Florida Division of Administrative Hearings',
  'Florida House of Representatives',
  'Florida Legislature',
  'Florida Office of Financial Regulation',
  'Florida Office of Insurance Regulation',
  'Florida Senate',
  'Fondo Bancario de Protección al Ahorro',
  'Foreign & Commonwealth Office',
  'Foreign Exchange Commission',
  'Foundation Germany in Plus',
  'French Advertising Standards Authority',
  'French Banking Federation',
  'French Competition Authority',
  'French Constitutional Council',
  'French Cybersecurity Agency',
  'French General Directorate for Competition, Consumer Affairs and Fraud Control',
  'French Government',
  'French Ministry for the Economy and Finance',
  'French Parliament',
  'French President of the Republic',
  'G20',
  'Gabon Deposits and Consignments Fund',
  'GARDA National Economic Crime Bureau',
  'Gazette of India',
  'General Assembly of Uruguay',
  'General Directorate of Consumer Affairs',
  'General Directorate of Consumer Protection and Market Surveillance - Turkey',
  'General Inspector of Financial Information',
  'General Secretariat for the Economy',
  'General Secretariat of the Treasury and Financial Policy',
  'General Superintendency of Securities of Costa Rica',
  'General Superintendent of Financial Entities - Costa Rica',
  'Georgia Attorney General',
  'Georgia Consumer Protection Division',
  'Georgia Data Exchange Agency',
  'Georgia Department of Banking and Finance',
  'Georgia Department of Economic Development',
  'Georgia General Assembly',
  'Georgia Governor',
  'Georgia Legislature',
  'Georgia Ministry of Economy and Sustainable Development',
  'Georgia Office of the Commissioner of Insurance and Fire Safety',
  'Georgia Personal Data Protection Service',
  'Georgia Secretary of State',
  'Georgia Service for Accounting, Reporting and Auditing Supervision (SARAS)',
  'German Banking Industry Committee',
  'German Federal Cartel Office',
  'German Federal Criminal Police Office',
  'German Federal Data Ethics Commission',
  'German Federal Financial Supervisory Authority',
  'German Federal Government (Bundestag)',
  'German Federal Office for Information Security',
  'German Government',
  'German Savings Banks Association',
  'Gibraltar Financial Intelligence Unit',
  'Gibraltar Financial Services Commission',
  'Gibraltar Government',
  'Gibraltar Regulatory Authority',
  'Global Financial Innovation Network',
  'Government of British Columbia',
  'Government of China',
  'Government of Croatia',
  'Government of Curaçao',
  'Government of Dubai',
  'Government of Ghana',
  'Government of Gibraltar',
  'Government of Greece',
  'Government of Guyana',
  'Government of India',
  'Government of Kenya',
  'Government of Luxembourg',
  'Government of Macau',
  'Government of Montenegro',
  'Government of Netherlands',
  'Government of Pakistan',
  'Government of Principality of Liechtenstein',
  'Government of Russia',
  'Government of Singapore',
  'Government of Slovakia',
  'Government of Slovenia',
  'Government of Sri Lanka',
  'Government of Thailand',
  'Government of the Cayman Islands',
  'Government of the Cook Islands',
  'Government of the Czech Republic',
  'Government of the Federated States of Micronesia',
  'Government of the Federation of Bosnia and Herzegovina',
  'Government of the Republic of North Macedonia',
  'Government of the UAE',
  'Government of the US Virgin Islands',
  'Government of Ukraine',
  'Government Offices of Sweden',
  'Government Technology Agency of Singapore',
  'Governor of Connecticut',
  'Governor of Florida',
  'Governor of Kansas',
  'Governor of Louisiana',
  'Governor of Maine',
  'Governor of Massachusetts',
  'Governor of Mississippi',
  'Governor of Missouri',
  'Governor of Nevada',
  'Governor of New Hampshire',
  'Governor of New Mexico',
  'Governor of New York',
  'Governor of North Carolina',
  'Governor of North Dakota',
  'Governor of Oklahoma',
  'Governor of Oregon',
  'Grand National Assembly of Turkey',
  'Greece National Printing House',
  'Greece Open Government Initiative',
  'Greek Anti-Money Laundering Authority',
  'Greek Independent Authority for Public Revenue',
  'Greek Ministry of Finance',
  'Greek Ministry of Justice, Transparency and Human Rights',
  'Hawai\'i Legislature',
  'Hawaii Attorney General',
  'Hawaii Department of Business, Economic Development & Tourism',
  'Hawaii Department of Commerce and Consumer Affairs',
  'Hawaii Department of Commerce and Consumer Affairs - Division of Financial Institutions',
  'Hawaii Executive Director of the Office of Consumer Protection',
  'Hawaii Governor',
  'Hawaii Office of Consumer Protection',
  'Hawaii Office of Enterprise Technology Services',
  'Head of the Cabinet of Ministers of Argentina',
  'Hellenic Bank Association',
  'Hellenic Capital Market Commission',
  'Hellenic Competition Commission',
  'Hellenic Data Protection Authority',
  'Hellenic Parliament',
  'Helsinki Fintech Farm',
  'High Council for Financial Stability',
  'High Court of Auckland',
  'High Court of Australia',
  'HM Revenue & Customs',
  'HM Treasury',
  'Home Office',
  'Honduras Finance Secretary',
  'Honduras General Directorate for Consumer Protection',
  'Honduras Institute for Access to Public Information',
  'Honduras National Commission of Banking and Insurance',
  'Hong Kong Association of Banks',
  'Hong Kong Association of Restricted Licence Banks and Deposit-taking Companies',
  'Hong Kong Competition Commission',
  'Hong Kong Computer Emergency Response Team',
  'Hong Kong Consumer Council',
  'Hong Kong Customs and Excise Department',
  'Hong Kong Federation of E-Commerce',
  'Hong Kong Government',
  'Hong Kong Institute for Monetary and Financial Research',
  'Hong Kong Mandatory Provident Fund Schemes Authority',
  'Hong Kong Monetary Authority',
  'Hong Kong Money Service Operators Association',
  'Hørings oversigt',
  'House of Assembly - New Hampshire',
  'House of Assembly - Ohio',
  'House of Commons - UK',
  'House of Commons of Canada',
  'House of Councillors',
  'House of Representatives of Thailand',
  'House of Representatives',
  'House of Representatives - Pennsylvania',
  'House of Representatives of Cyprus',
  'House of Representatives of the Netherlands',
  'Hungarian Banking Association',
  'Hungarian Competition Authority',
  'Hungarian Financial Arbitration Board',
  'Hungarian Government',
  'Hungarian National Authority for Data Protection and Freedom of Information',
  'Hungarian National Tax and Customs Administration',
  'Hungarian Parliament',
  'Iceland Competition Authority',
  'Iceland Financial Stability Committee',
  'Iceland Financial Supervision Authority',
  'Icelandic Competition Authority',
  'Icelandic Government',
  'Icelandic Parliament',
  'Idaho Attorney General',
  'Idaho Department of Finance',
  'Idaho Governor',
  'Idaho Legislature',
  'Illinois Attorney General',
  'Illinois Department of Commerce and Economic Activity',
  'Illinois Department of Financial and Professional Regulation',
  'Illinois Department of Innovation and Technology',
  'Illinois Department of Insurance',
  'Illinois Department of Securities',
  'Illinois Governor',
  'Illinois Legislature',
  'Independent Authority for Public Revenue (IAPR)',
  'Independent Public Revenue Authority',
  'Independent Public Revenue Authority (A.A.D.E)',
  'India Department for Promotion of Industry and Internal Trade',
  'India Department of Commerce',
  'India Department of Financial Services',
  'Indian Banks Board Bureau',
  'Indian Banks\' Association',
  'Indian Computer Emergency Response Team',
  'Indian Department of Consumer Affairs',
  'Indian Government',
  'Indian Institute of Banking And Finance',
  'Indian Parliament',
  'Indiana Attorney General',
  'Indiana Consumer Protection Division',
  'Indiana Department of Financial Institutions',
  'Indiana General Assembly',
  'Indiana Governor',
  'Indiana House of Representatives',
  'Indiana Legislature',
  'Indiana Securities Division',
  'Indiana Senate',
  'Indonesia Corruption Eradication Commission',
  'Indonesia Financial Services Authority',
  'Indonesian Competition Commission',
  'Indonesian Financial Transaction Reporting and Analysis Centre',
  'Indonesian Government',
  'Info-communications Media Development Authority of Singapore',
  'Infocomm Media Development Authority - Singapore',
  'Information and Communication Technology Agency of Sri Lanka',
  'Information and Data Protection Commissioner',
  'Information and Privacy Commission of Ontario',
  'Information and Privacy Commissioner of Ontario',
  'Information Commissioner of the Republic of Slovenia',
  'Information Commissioner\'s Office',
  'Information Regulator',
  'Information Technology Association of Canada',
  'Information Technology Industry Council',
  'Information Technology Promotion Agency',
  'Inland Revenue Authority of Singapore',
  'Innovation and Technology Commission',
  'Innovation, Science and Economic Development Canada',
  'Insolvency and Bankruptcy Board of India',
  'Institute for Development and Research in Banking Technology',
  'Institute for Financial Services',
  'Institute for the Supervision of Insurance',
  'Institution of the Ombudsman for Consumer Protection',
  'Insurance Information Bureau of India',
  'Insurance Regulatory and Development Authority',
  'Internal Revenue Service',
  'International Finance Corporation',
  'International Finance Corporation (IFC) - Laos',
  'International Financial Services Centres Authority',
  'International Monetary Fund',
  'International Organization of Securities Commissions (IOSCO)',
  'Internet and Mobile Association of India',
  'Interpol',
  'Investment and Business Development Agency',
  'Iowa Attorney General',
  'Iowa Division of Banking',
  'Iowa Economic Development Authority',
  'Iowa Governor',
  'Iowa House of Representatives',
  'Iowa Insurance Division',
  'Iowa Legislature',
  'Iowa Senate',
  'Irish Competition and Consumer Protection Commission',
  'Irish Criminal Assets Bureau',
  'Irish Data Protection Commission',
  'Irish Department of Enterprise, Trade and Employment',
  'Irish Department of Finance',
  'Irish Department of Justice and Equality',
  'Irish Deposit Guarantee Scheme',
  'Irish Government',
  'Irish Parliament',
  'Irish Reporting and Information Security Service',
  'Irish Takeover Panel',
  'IRISS-CERT',
  'Isle of Man Financial Services Authority',
  'Israel Competition Authority',
  'Israel Consumer Protection and Fair Trade Authority (ICPFTA)',
  'Israel Money Laundering and Terror Financing Prohibition Authority',
  'Israel Privacy Protection Authority',
  'Israel Securities Authority',
  'Israeli Capital Market, Insurance and Savings Authority',
  'Israeli Government',
  'Israeli Parliament',
  'Istituto dell\'Autodisciplina Pubblicitaria (IAP)',
  'Italian Association for Information Security',
  'Italian Association Responsible for Anti-Money Laundering',
  'Italian Banking Association',
  'Italian Chamber of Deputies',
  'Italian Communications Authority (AGCOM)',
  'Italian Competition Authority',
  'Italian Council of Ministers',
  'Italian Data Protection Authority',
  'Italian Financial CERT',
  'Italian Government',
  'Italian National Commission for Companies and the Stock Exchange',
  'Italian Organism of Agents and Brokers',
  'Italian Parliament',
  'Italian Payments Committee',
  'Italian Revenue Agency',
  'Ivory Coast Telecommunications Regulatory Authority',
  'Jabatan Perlindungan Data Peribadi',
  'Japan Advertising Review Organisation',
  'Japan Association of New Economy',
  'Japan Business Federation',
  'Japan Chamber of Commerce and Industry',
  'Japan Computer Security Incident Response Team Coordination Center',
  'Japan Consumer Credit Association',
  'Japan Crypto Asset Trading Association',
  'Japan Crypto Business Association',
  'Japan Cryptocurrency Exchange Association (JVCEA)',
  'Japan Fair Trade Commission',
  'Japan Financial Intelligence Center',
  'Japan Financial Intelligence Unit',
  'Japan House of Representatives',
  'Japan National Cybersecurity Office',
  'Japan Securities Dealers Association',
  'Japan Virtual and Crypto Assets Exchange Association',
  'Japan Virtual Currency Exchange Association',
  'Japanese Bankers Association',
  'Japanese Cabinet Office',
  'Japanese Consumer Affairs Agency',
  'Japanese Government',
  'Japanese Industrial Standards Committee',
  'Japanese National Diet',
  'Japanese Virtual Currency Business Association',
  'Joint Financial Intelligence Unit',
  'Joint Money Laundering Steering Group',
  'Joint Regulatory Oversight Committee',
  'Jordan Securities Commission',
  'Kansalaisaloite',
  'Kansas Attorney General',
  'Kansas Department of Credit Unions',
  'Kansas Department of Insurance',
  'Kansas House of Representatives',
  'Kansas Legislature',
  'Kansas Office of the State Bank Commissioner',
  'Kansas Secretary of State',
  'Kansas Senate',
  'Kansas State Office of Information Technology Services',
  'Kazakhstan Agency for Protection of Competition',
  'Kazakhstan Agency for Regulation and Development of Financial Market',
  'Kazakhstan Government',
  'Kazakhstan Ministry of Digital Development, Innovation and Aerospace Industry',
  'Kazakhstan Parliament',
  'Kentucky Attorney General',
  'Kentucky Cabinet for Economic Development',
  'Kentucky Department of Financial Institutions',
  'Kentucky General Assembly',
  'Kentucky Governor',
  'Kentucky Legislature',
  'Kentucky Public Protection Cabinet',
  'Kentucky Senate',
  'Kentucky Transportation Cabinet',
  'Kenya Bankers Association',
  'Kenya Ethics and Anti-Corruption Commission',
  'Kenya National Council for Law Reporting',
  'Kenya Office of the Data Protection Commissioner',
  'Korean Consumer Agency',
  'Korean Cyber Security Center',
  'Korean Financial Intelligence Unit',
  'Kosovo Assembly',
  'Kosovo Government',
  'Krajowa Izba Rozliczeniowa',
  'Kyrgyzstan Financial Intelligence Unit',
  'Kyrgyzstan Ministry of Economy and Commerce',
  'Kyrgyzstan State Agency for Protection of Personal Data',
  'L\'Agence Nationale de la Sécurité des Systèmes d\'Information (ANSSI)',
  'Labuan Financial Services Authority',
  'Landtag',
  'Lanka Clear',
  'Laos Anti-Money Laundering Office',
  'Laos Ministry of Technology and Communications',
  'Latvia Cabinet of Ministers',
  'Latvia Competition Council',
  'Latvia Data State Inspectorate',
  'Latvian Finance and Capital Market Commission',
  'Latvian Financial Intelligence Unit',
  'Latvian Government',
  'Latvian Parliament',
  'Latvijas Banka (Central Bank of Latvia)',
  'Law Commission of India',
  'Law Commission of the United Kingdom',
  'Law Net - Ministry of Justice',
  'Law Reform Commission of Ireland',
  'LawTech Delivery Panel',
  'Lebanese Government',
  'Lebanese Parliament',
  'Legislative Assembly of British Columbia',
  'Legislative Assembly of Costa Rica',
  'Legislative Assembly of El Salvador',
  'Legislative Assembly of Macau',
  'Legislative Assembly of Puerto Rico',
  'Legislative Assembly of Tonga',
  'Legislative Council of the Hong Kong SAR',
  'Legislative Yuan',
  'Legislature of Guam',
  'Legislature of the Commonwealth of Northern Mariana Islands',
  'Legislature of the Virgin Islands',
  'Lending Standards Board',
  'Liechtenstein Data Protection Office',
  'Liechtenstein National Administration',
  'Liechtenstein State Administration',
  'Life Insurance Council',
  'Lithuania Bank Association',
  'Lithuanian Government',
  'Lithuanian Ministry of Economy and Innovation',
  'Lithuanian Ministry of Finance',
  'Lithuanian Ministry of Justice',
  'Lithuanian Parliament',
  'Lithuanian State Budgetary Institution',
  'Lithuanian State Consumer Rights Protection Service',
  'Lithuanian State Data Protection Inspectorate',
  'Lok Sabha - House of the People',
  'Louisiana Attorney General',
  'Louisiana House of Representatives',
  'Louisiana Legislature',
  'Louisiana Office of Financial Institutions',
  'Louisiana Senate',
  'Louisiana State Treasurer',
  'Luxembourg Banker\'s Association',
  'Luxembourg Bar Association',
  'Luxembourg Chamber of Commerce',
  'Luxembourg Chamber of Deputies',
  'Luxembourg Chamber of Notaries',
  'Luxembourg Consumer Protection Association',
  'Luxembourg Consumers Union',
  'Luxembourg Government',
  'Luxembourg Institute of Auditors',
  'Luxembourg Ministry of Foreign and European Affairs',
  'Luxembourgish Competition Authority',
  'Luxembourgish Data Protection Authority',
  'Macau Economic and Technological Development Bureau',
  'Macau Financial Intelligence Office',
  'Magyar Nemzeti Bank',
  'Maine Attorney General',
  'Maine Bureau of Consumer Credit Protection',
  'Maine Bureau of Financial Institutions',
  'Maine Department of Economic and Community Development',
  'Maine Governor',
  'Maine House of Representatives',
  'Maine Legislature',
  'Maine Office of Securities',
  'Maine Senate',
  'Malaysia House of Commons',
  'Malaysia Ministry of Commerce',
  'Malaysia Securities Commission',
  'Malaysia Senate',
  'Malaysian Government',
  'Malaysian Ministry of Finance',
  'Maldives Government',
  'Maldives Monetary Authority',
  'Maldives Parliament',
  'Maldives Payment System Development Project',
  'Mali Personal Data Protection Authority',
  'Malta Competition and Consumer Affairs Authority (MCCAA)',
  'Malta Cybersecurity National Coordination Centre',
  'Malta Digital Innovation Authority',
  'Malta Financial Services Advisory Council',
  'Malta Financial Services Authority',
  'Malta Government',
  'Malta Sanctions Monitoring Board',
  'Maltese Government',
  'Maryland Attorney General',
  'Maryland Cybersecurity Council',
  'Maryland Department of Labor',
  'Maryland General Assembly',
  'Maryland Governor',
  'Maryland House of Representatives',
  'Maryland Legislature',
  'Maryland Office of Financial Regulation',
  'Maryland Senate',
  'Maryland State Police',
  'Massachusetts Attorney General',
  'Massachusetts Division of Banks',
  'Massachusetts Government',
  'Massachusetts House of Representatives',
  'Massachusetts Securities Division',
  'Massachusetts Senate',
  'Mauritius Government',
  'Mauritius National Assembly',
  'Mayor of Washington DC',
  'Metropolitan Chamber of Commerce & Industry - Bangladesh',
  'Mexican Chamber of Deputies',
  'Mexican Federal Competition Commission',
  'Mexican Federal Government',
  'Mexican Federal Prosecutor for the Consumer',
  'Mexican General Congress',
  'Michigan Attorney General',
  'Michigan Department of Insurance and Financial Services',
  'Michigan Governor',
  'Michigan Legislature',
  'MicroFinance Foundation',
  'Minister of the Interior - Nicaragua',
  'Ministère de la Justice (Ministry of Justice)',
  'Ministry for Economic Affairs and Energy - Germany',
  'Ministry for Foreign Affairs - Finland',
  'Ministry for Public Expenditure and Reform - Ireland',
  'Ministry of Agriculture, Forestry and Fisheries - Japan',
  'Ministry of Business, Innovation and Employment - New Zealand',
  'Ministry of Civil Affairs',
  'Ministry of Commerce - Saudi Arabia',
  'Ministry of Communication and Digital of the Republic of Indonesia',
  'Ministry of Communication and IT',
  'Ministry of Communications and Information - Singapore',
  'Ministry of Communications and Information Technology - Egypt',
  'Ministry of Constitutional Affairs, Reforms, Deregulation and Justice - Austria',
  'Ministry Of Consumer Affairs, Food and Public Distribution',
  'Ministry of Corporate Affairs - India',
  'Ministry of Culture - Norway',
  'Ministry of Defence - Germany',
  'Ministry of Digital Affairs - Poland',
  'Ministry of Digital Economy and Society',
  'Ministry of Digital Transformation - North Macedonia',
  'Ministry of Economic Affairs - Taiwan',
  'Ministry of Economic Affairs and Climate Policy - Netherlands',
  'Ministry of Economic Affairs and Digital Transformation',
  'Ministry of Economic Affairs and Employment - Finland',
  'Ministry of Economic Development - Italy',
  'Ministry of Economy - Abu Dhabi',
  'Ministry of Economy - Argentina',
  'Ministry of Economy - Brazil',
  'Ministry of Economy - Lebanon',
  'Ministry of Economy and Business - Spain',
  'Ministry of Economy and Finance - Italy',
  'Ministry of Economy and Finance - Panama',
  'Ministry of Economy and Finance - Peru',
  'Ministry of Economy and Finance - Uruguay',
  'Ministry of Economy and Finance of Paraguay',
  'Ministry of Economy and Industry',
  'Ministry of Economy, Finance and Industrial and Digital Sovereignty - France',
  'Ministry of Economy, Finance of Industrial and Digital Sovereignty - France',
  'Ministry of Economy, Finance, and Development - Burkina Faso',
  'Ministry of Economy, Trade and Enterprise',
  'Ministry of Economy, Trade and Industry - Japan',
  'Ministry of Electronics and Information Technology of India',
  'Ministry of Enterprise and Innovation - Sweden',
  'Ministry of Finance - Albania',
  'Ministry of Finance - Austria',
  'Ministry of Finance - Bangladesh',
  'Ministry of Finance - Belgium',
  'Ministry of Finance - Bulgaria',
  'Ministry of Finance - Chile',
  'Ministry of Finance - Comoros',
  'Ministry of Finance - Costa Rica',
  'Ministry of Finance - Croatia',
  'Ministry of Finance - Curaçao',
  'Ministry of Finance - Cyprus',
  'Ministry of Finance - Czech Republic',
  'Ministry of Finance - Denmark',
  'Ministry of Finance - DR Congo',
  'Ministry of Finance - Estonia',
  'Ministry of Finance - Finland',
  'Ministry of Finance - Hungary',
  'Ministry of Finance - India',
  'Ministry of Finance - Indonesia',
  'Ministry of Finance - Ireland',
  'Ministry of Finance - Israel',
  'Ministry of Finance - Japan',
  'Ministry of Finance - Laos',
  'Ministry of Finance - Luxembourg',
  'Ministry of Finance - Nepal',
  'Ministry of Finance - Netherlands',
  'Ministry of Finance - North Macedonia',
  'Ministry of Finance - Norway',
  'Ministry of Finance - Pakistan',
  'Ministry of Finance - Paraguay',
  'Ministry of Finance - Romania',
  'Ministry of Finance - Russia',
  'Ministry of Finance - Saudi Arabia',
  'Ministry of Finance - Singapore',
  'Ministry of Finance - Slovenia',
  'Ministry of Finance - Sri Lanka',
  'Ministry of Finance - Sweden',
  'Ministry of Finance - Taiwan',
  'Ministry of Finance - Thailand',
  'Ministry of Finance - Vietnam',
  'Ministry of Finance and Budget - Central African Republic',
  'Ministry of Finance and Economic Management - Cook Islands',
  'Ministry of Finance and Economy - Brunei',
  'Ministry of Finance and Public Credit - Mexico',
  'Ministry of Finance and Public Credit of Colombia',
  'Ministry of Finance of the Republic of Latvia',
  'Ministry of Finance of the Slovak Republic',
  'Ministry of Foreign Affairs - Japan',
  'Ministry of Health, Labour and Welfare - Japan',
  'Ministry of High-Technological Industry of the Republic of Armenia',
  'Ministry of Home Affairs - Singapore',
  'Ministry of Industry and Commerce - Laos',
  'Ministry of Industry and Trade - Vietnam',
  'Ministry of Industry, Business and Financial Affairs - Denmark',
  'Ministry of Information, Communications and Technology - Kenya',
  'Ministry of Innovation and Technology',
  'Ministry of Interior - Czech Republic',
  'Ministry of Interior - Saudi Arabia',
  'Ministry of Internal Affairs - Romania',
  'Ministry of Internal Affairs and Communications - Japan',
  'Ministry of Investments, Regional Development and Informatization of the Slovak Republic',
  'Ministry of Justice - Denmark',
  'Ministry of Justice - Finland',
  'Ministry of Justice - France',
  'Ministry of Justice - Gabon',
  'Ministry of Justice - Hungary',
  'Ministry of Justice - Israel',
  'Ministry of Justice - Japan',
  'Ministry of Justice - Mali',
  'Ministry of Justice - Netherlands',
  'Ministry of Justice - New Zealand',
  'Ministry of Justice - Norway',
  'Ministry of Justice - Russia',
  'Ministry of Justice - Slovenia',
  'Ministry of Justice - Taiwan',
  'Ministry of Justice and Human Rights - Argentina',
  'Ministry of Justice and Human Rights - Central African Republic',
  'Ministry of Justice and Human Rights - Peru',
  'Ministry of Justice Anti-Money Laundering Division - Taiwan',
  'Ministry of Justice of the Republic of Latvia',
  'Ministry of Justice of the Republic of Uzbekistan',
  'Ministry of Justice of the Slovak Republic',
  'Ministry of Justice, Administration and Digital Transformation, Office for Legislation - Croatia',
  'Ministry of Labour, Social Affairs and Consumer Protection - Austria',
  'Ministry of Law and Justice - India',
  'Ministry of Local Government and Modernisation - Norway',
  'Ministry of Posts and Telecommunications - Central African Republic',
  'Ministry of Posts, Telecommunications, and Digital Economy',
  'Ministry of Public Security - Vietnam',
  'Ministry of Security of Bosnia and Herzegovina',
  'Ministry of Social Affairs and Employment - Netherlands',
  'Ministry of Social Affairs and Health - Finland',
  'Ministry of the Economy, Tourism and Sport - Slovenia',
  'Ministry of the Interior',
  'Ministry of the Interior - Finland',
  'Ministry of the Interior - France',
  'Ministry of the Interior of the Slovak Republic',
  'Ministry of Trade - Indonesia (BAPPEBTI)',
  'Ministry of Trade and Economic Development',
  'Ministry of Trade and Industry - Singapore',
  'Ministry of Trade, Tourism, and Telecommunications',
  'Ministry of Treasury and Finance - Turkey',
  'Minnesota Attorney General',
  'Minnesota Department of Commerce',
  'Minnesota Governor',
  'Minnesota House of Representatives',
  'Minnesota Legislature',
  'Minnesota Senate',
  'Mississippi Attorney General',
  'Mississippi Department of Banking and Consumer Finance',
  'Mississippi House of Representatives',
  'Mississippi Legislature',
  'Mississippi Secretary of State',
  'Mississippi Senate',
  'Missouri Attorney General',
  'Missouri Commissioner of Securities',
  'Missouri Division of Finance',
  'Missouri House of Representatives',
  'Missouri Legislature',
  'Missouri Secretary of State',
  'Missouri Senate',
  'Moldova Ministry of Economic Development and Digitalization',
  'Moldova Office for Prevention and Fight Against Money Laundering',
  'Monetary Authority of Macau',
  'Monetary Authority of Singapore',
  'Money Laundering Reporting Office Switzerland',
  'MONEYVAL',
  'Montana Banking & Financial Institutions Division',
  'Montana Commissioner of Securities and Insurance',
  'Montana Department of Justice',
  'Montana Governor',
  'Montana House of Representatives',
  'Montana Secretary of State',
  'Montana Senate',
  'Montana State Legislature',
  'Montenegro Capital Market Authority',
  'Namibia Financial Institutions Supervisory Authority',
  'Namibia Financial Intelligence Centre',
  'Namibia Ministry of Information and Communication Technology',
  'Národná banka Slovenska',
  'Narodowy Bank Polski',
  'National Agency for Regulation in Electronic Communications and Information Technology',
  'National Agency for the Protection of Personal Data',
  'National Anti-Corruption Commission',
  'National Anti-Money Laundering and Combating Financing of Terrorism and Financing of Illegal Organisations Committee',
  'National Assembly - Bahrain',
  'National Assembly - France',
  'National Assembly - Hungary',
  'National Assembly - Slovenia',
  'National Assembly - Thailand',
  'National Assembly (Panama)',
  'National Assembly of Armenia',
  'National Assembly of Pakistan',
  'National Assembly of Quebec',
  'National Assembly of Republika Srpska',
  'National Assembly of South Korea',
  'National Authority for Data Protection and Freedom of Information',
  'National Authority for Transparency and Access to Information',
  'National Authority for Transparency and Access to Information - Panama',
  'National Bank for Agriculture and Rural Development',
  'National Bank of Angola',
  'National Bank of Belgium',
  'National Bank of Cambodia',
  'National Bank of Georgia',
  'National Bank of Kazakhstan',
  'National Bank of Moldova',
  'National Bank of Panama',
  'National Bank of Romania',
  'National Bank of Serbia',
  'National Bank of Slovakia',
  'National Bank of Tajikistan',
  'National Bank of the Kyrgyz Republic',
  'National Bank of the Republic of Belarus',
  'National Bank of the Republic of North Macedonia',
  'National Bank of Ukraine',
  'National Banking and Securities Commission (CNBV)',
  'National Bureau of Investigation - Financial Intelligence Unit',
  'National Cashless Payments Committee',
  'National Center for Cyber Security Technology',
  'National Center for Cybersecurity',
  'National Center for Personal Data Protection of the Republic of Moldova',
  'National Center of Incident Readiness and Strategy for Cybersecurity',
  'National Commission for Competition Defense',
  'National Commission for Data Protection',
  'National Commission for the Defense of Competition (CNDC)',
  'National Commission for the Protection of Users of Financial Services',
  'National Commission for the Retirement Savings System',
  'National Commission on Markets and Competition - Spain',
  'National Communications Commission',
  'National Conference of State Legislatures',
  'National Congress of Argentina',
  'National Congress of Honduras',
  'National Consumer Commission',
  'National Consumer Disputes Redressal Commission',
  'National Consumer Disputes Redressal Commission of India',
  'National Consumer Protection Agency',
  'National Council - Austria',
  'National Council of the Slovak Republic',
  'National Credit Card Center of R.O.C',
  'National Credit Regulator',
  'National Credit Union Administration',
  'National Crime Agency',
  'National Cyber Security Centre - Ireland',
  'National Cyber Security Centre - New Zealand',
  'National Cyber Security Centre - United Kingdom',
  'National Cybersecurity Agency of France',
  'National Cybersecurity Agency of the Italian Republic',
  'National Data Privacy Commission',
  'National Data Protection Authority (ANPD)',
  'National Development and Reform Commission',
  'National Development Bank of Palau',
  'National Development Council',
  'National Digital Assets Commission (CNAD)',
  'National Digital ID Company Limited',
  'National Financial Intelligence Unit - Venezuela',
  'National Financial Market Commission of Moldova',
  'National Financial Regulatory Administration',
  'National Futures Association',
  'National Information & Communication Security Taskforce',
  'National Information Security Standardisation Technical Committee',
  'National Information Technology Development Agency',
  'National Institute for the Defence of Competition and the Protection of Intellectual Property',
  'National Institute for the Defence of Competition and the Protection of Intellectual Property (INDECOPI)',
  'National Institute of Cyber Security',
  'National Institute of Standards and Technology',
  'National Institute of Transparency, Access to Information and Protection of Personal Data',
  'National Institution for Transforming India',
  'National Internet Association of China',
  'National Legislative Assembly of the Lao PDR',
  'National Monetary Council',
  'National Office for Cyber and Information Security - Czech Republic',
  'National Parliament of Papua New Guinea',
  'National Payments Committee',
  'National Payments Corporation of India',
  'National People\'s Congress',
  'National Police Agency',
  'National Reserve Bank of Tonga',
  'National Securities and Stock Market Commission of Ukraine',
  'National Securities Commission of Argentina',
  'National Security and Exchange Commission',
  'National Standards Authority of Ireland',
  'National Superintendency of Customs and Tax Administration',
  'National Tax and Customs Agency - Argentina',
  'National Treasury of Kenya',
  'National Union of Credit Cooperatives - Spain',
  'Nationwide Multistate Licensing System',
  'NCE Finance Innovation',
  'NCSC-FI',
  'Nebraska Attorney General',
  'Nebraska Department of Banking and Finance',
  'Nebraska Governor',
  'Nebraska State Legislature',
  'Nepal Financial Institution Supervision Departments',
  'Nepal Law Commission',
  'Nepal Rastra Bank',
  'Nevada Attorney General',
  'Nevada Department of Business and Industry',
  'Nevada Financial Institutions Division',
  'Nevada Legislature',
  'Nevada Secretary of State',
  'Nevada Securities Division',
  'New Hampshire Banking Department',
  'New Hampshire Department of Justice',
  'New Hampshire Legislature',
  'New Hampshire Secretary of State',
  'New Jersey Attorney General',
  'New Jersey Bureau of Securities',
  'New Jersey Cyber Fraud Unit',
  'New Jersey Cybersecurity and Communications Integration Cell',
  'New Jersey Department of Banking and Insurance',
  'New Jersey Legislature',
  'New Jersey Office of Consumer Protection',
  'New Mexico Department of Justice',
  'New Mexico Financial Institutions Division',
  'New Mexico Legislature',
  'New Mexico Regulation and Licensing Department',
  'New Mexico Securities Division',
  'New Payments Platform',
  'New York Attorney General',
  'New York Department of State',
  'New York Senate',
  'New York State Assembly',
  'New York State Department of Financial Services',
  'New York State Legislature',
  'New Zealand Bankers\' Association',
  'New Zealand Commerce Commission',
  'New Zealand Cyber Security Centre',
  'New Zealand Department of Internal Affairs',
  'New Zealand Government',
  'New Zealand Ministry of Foreign Affairs and Trade',
  'New Zealand Parliamentary Counsel Office',
  'New Zealand Serious Fraud Office',
  'Nicaragua Financial Analysis Unit',
  'Nicaragua National Assembly',
  'Nicaragua National Institute for the Promotion of Competition',
  'Nicaragua Superintendency of Banks and Other Financial Institutions',
  'Nigeria Computer Emergency Response Team',
  'Nigeria Data Protection Commission',
  'Nigerian Communications Commission',
  'Nigerian Federal Government',
  'Nigerian Financial Intelligence Unit',
  'Nigerian National Assembly',
  'Nordic Payments Council',
  'Norges Bank',
  'North Carolina Attorney General',
  'North Carolina Commissioner of Banks',
  'North Carolina Department of Justice',
  'North Carolina Legislature',
  'North Carolina Secretary of State',
  'North Carolina Securities Division',
  'North Dakota Attorney General',
  'North Dakota Department of Financial Institutions',
  'North Dakota Legislature',
  'North Dakota Securities Department',
  'North Macedonia State Market Inspectorate',
  'Norway National Authority for Investigation and Prosecution of Economic and Environmental Crime',
  'Norwegian Consumer Complaints Committee',
  'Norwegian Consumer Council',
  'Norwegian Data Protection Authority',
  'Norwegian Directorate for Export Control and Sanctions',
  'Norwegian Finance Appeals Committee',
  'Norwegian Financial Advisory Board',
  'Norwegian Finanstilsynet',
  'Norwegian Government',
  'Norwegian Ministry of Trade, Industry and Fisheries',
  'Norwegian Parliament',
  'Norwegian Police Directorate',
  'NZ Financial Intelligence Unit',
  'Observatory for the Security of Payment Means',
  'Oesterreichische Nationalbank',
  'Office for Personal Data Protection - Czech Republic',
  'Office for Personal Data Protection - Macau',
  'Office for Prevention of Transfer of Criminal Proceeds - Japan',
  'Office for the Protection of Competition - Czech Republic',
  'Office of Competition and Consumer Protection - Poland',
  'Office of Data Protection - Abu Dhabi Global Market',
  'Office of Financial Sanctions Implementation',
  'Office of Financial Sanctions Implementation (OFSI)',
  'Office of Foreign Assets Control',
  'Office of Information Technology, State of Alaska',
  'Office of the Australian Information Commissioner',
  'Office of the Commissioner of Financial Institutions',
  'Office of the Communications Authority',
  'Office of the Comptroller of the Currency',
  'Office of the Consumer Protection Board',
  'Office of the Consumer Protection Board - Thailand',
  'Office of the Data Protection Ombudsman',
  'Office of the Director of Corporate Enforcement',
  'Office of the Executive Council of Insurers',
  'Office of the Financial Arbitrator',
  'Office of the Government Chief Information Officer of Hong Kong',
  'Office of the Governor of Alabama',
  'Office of the Information and Privacy Commissioner for British Columbia',
  'Office of the National Security Adviser of Nigeria',
  'Office of the Official Information Commission',
  'Office of the Premier of Ontario',
  'Office of the Prime Minister of Kosovo',
  'Office of the Privacy Commissioner of Canada',
  'Office of the Privacy Commissioner of New Zealand',
  'Office of the Superintendent of Financial Institutions',
  'Official State Gazette',
  'Ohio Attorney General',
  'Ohio Department of Administrative Services',
  'Ohio Department of Commerce/Division of Securities',
  'Ohio Department of Insurance',
  'Ohio Governor',
  'Ohio Legislature',
  'Oklahoma Attorney General',
  'Oklahoma Banking Department',
  'Oklahoma Department of Consumer Credit',
  'Oklahoma Department of Securities',
  'Oklahoma Legislature',
  'ØKOKRIM',
  'Ombudsman for Banking Services and Investments - Canada',
  'Ombudsman of Ukraine',
  'Ontario Capital Markets Tribunal',
  'Ontario Legislative Assembly',
  'Ontario Securities Commission',
  'Open Banking Implementation Entity',
  'Open Banking Limited (OBL)',
  'Open Finance Association',
  'Open Finance Association (OFA)',
  'Operational and Analytical Center under the President of the Republic of Belarus',
  'Oregon Department of Consumer and Business Services',
  'Oregon Department of Justice',
  'Oregon Division of Financial Regulation',
  'Oregon Legislature',
  'Oregon Secretary of State',
  'Organisation for Economic Co-operation and Development',
  'Organismo Agenti e Mediatori (OAM)',
  'Overseas Issuing Institute - New Caledonia',
  'Pacific Financial Inclusion Program - Papua New Guinea',
  'Palau Financial Institutions Commission',
  'Palau Financial Intelligence Unit',
  'Palau Government',
  'Pan-African Payment & Settlement System',
  'Pan-Nordic Card Association',
  'Panama Consumer Protection and Competition Authority',
  'Panama Financial Analysis Unit',
  'Paraguay National Competition Commission',
  'Paraguay National Secretariat for Consumer and User Defense',
  'Paraguay Secretariat for the Prevention of Money Laundering and Asset Forfeiture',
  'Parlamento di Kòrsou',
  'Parliament of Australia',
  'Parliament of Canada',
  'Parliament of Cyprus',
  'Parliament of Egypt',
  'Parliament of Fiji',
  'Parliament of Finland',
  'Parliament of Georgia',
  'Parliament of Ghana',
  'Parliament of Gibraltar',
  'Parliament of Guyana',
  'Parliament of Iceland',
  'Parliament of Kenya',
  'Parliament of Malta',
  'Parliament of Montenegro',
  'Parliament of New Zealand',
  'Parliament of Pakistan',
  'Parliament of Samoa',
  'Parliament of Singapore',
  'Parliament of Solomon Islands',
  'Parliament of South Africa',
  'Parliament of Sri Lanka',
  'Parliament of the Czech Republic',
  'Parliament of the Maldives',
  'Parliament of the Republic of the Marshall Islands',
  'Parliament of the United Kingdom',
  'Parliament of Uruguay',
  'Parliament of Vanuatu',
  'Parliamentary Joint Committee On Corporations And Financial Services',
  'Parquet National Financier',
  'Pay.UK',
  'Payment Association of South Africa',
  'Payment Systems Regulator',
  'Payments Canada',
  'Payments Council of India',
  'Payments Japan',
  'Payments Network Malaysia (PayNet)',
  'Payments NZ',
  'Payments UK',
  'PCI Security Standards Council',
  'Pennsylvania Attorney General',
  'Pennsylvania Department of Banking and Securities (DoBS)',
  'Pennsylvania Governor\'s Office',
  'Pennsylvania Legislature',
  'PensionsEurope',
  'People\'s Bank of China',
  'People\'s Consultative Assembly',
  'Personal Data Protection Agency of North Macedonia',
  'Personal Data Protection Authority of the Kingdom Of Bahrain',
  'Personal Data Protection Commission - Singapore',
  'Personal Data Protection Commissioner\'s Office - Malaysia',
  'Personal Data Protection Committee - Thailand',
  'Personal Data Protection Office - Poland',
  'Personal Data Protection Office of the Slovak Republic',
  'Personal Information Protection Commission - Japan',
  'Personal Information Protection Commission - South Korea',
  'Peru Association of Banks',
  'Peru Autoridad Nacional de Protección de Datos Personales',
  'Peru Financial Intelligence Unit',
  'Peru Superintendency of the Securities Market',
  'Peru\'s Superintendency of Banking, Insurance and Private Pension Fund Administrators (SBS)',
  'Peruvian Superintendency of the Securities Market',
  'Philippine House of Representatives',
  'Philippine National Privacy Commission',
  'Philippines Anti-Money Laundering Council',
  'Philippines Government',
  'Philippines Securities and Exchange Commission',
  'Philippine Competition Commission',
  'Polish Council of Ministers',
  'Polish Government',
  'Polish Ministry of Finance',
  'Polish Parliament',
  'PolishAPI',
  'Portuguese Competition Authority',
  'Portuguese Data Protection Authority',
  'Portuguese Government',
  'Portuguese Parliament',
  'Portuguese Securities Market Commission (CMVM)',
  'President of Kenya',
  'President of Kosovo',
  'President of Paraguay',
  'President of Serbia',
  'President of the Maldives',
  'President of Ukraine',
  'President of Vietnam',
  'Prime Minister\'s Office - Finland',
  'Privacy Commissioner for Personal Data of Hong Kong',
  'Privy Council',
  'Productivity Commission',
  'Prosecutor General\'s Office of the Russian Federation',
  'Prudential Regulation Authority',
  'Public Agency of the Republic of Slovenia for the Protection of Competition',
  'Public Safety Canada',
  'Qatar Central Bank',
  'Qatar Financial Centre Regulatory Authority',
  'Quebec Access to Information Commission',
  'Quebec Consumer Protection Office',
  'Quebec Financial Markets Authority',
  'Rajya Sabha - Council of States',
  'Regional Banks Association of Japan',
  'Regional Council for Public Savings and Financial Markets',
  'Regulatory and Control Unit of Personal Data (URCDP)',
  'Regulatory Authority for Electronic Communications and Postal Services',
  'Republic of Serbia Securities Commission',
  'Reserve Bank Information Technology Private Limited',
  'Reserve Bank of Australia',
  'Reserve Bank of Fiji',
  'Reserve Bank of India',
  'Reserve Bank of New Zealand',
  'Reserve Bank of Tonga',
  'Reserve Bank of Vanuatu',
  'Retsinformation',
  'Revenue Commissioners',
  'Rhode Island Attorney General',
  'Rhode Island Department of Business Regulation',
  'Rhode Island Department of State',
  'Rhode Island Governor',
  'Rhode Island State Legislature',
  'Richmond Federal Reserve',
  'Riksdag',
  'Romania Competition Authority',
  'Romania Competition Council',
  'Romanian Government',
  'Romanian Ministry of Economy',
  'Romanian National Authority for Consumer Protection',
  'Romanian National Office for Prevention and Control of Money Laundering',
  'Romanian National Supervisory Authority for Personal Data Processing',
  'Romanian Parliament',
  'Royal Monetary Authority of Bhutan',
  'Russian Electronic Money and Remittance Association',
  'SAARC Payments Initiative',
  'Sanctions Monitoring Board',
  'Saudi Arabia Capital Market Authority',
  'Saudi Arabia Financial Intelligence Unit',
  'Saudi Arabia General Authority for Competition',
  'Saudi Arabia General Authority for Media Regulation',
  'Saudi Arabia Ministry of Commerce',
  'Saudi Arabia National Center for Documents and Archives',
  'Saudi Arabian Government',
  'Saudi Central Bank',
  'Saudi Data and Artificial Intelligence Authority',
  'Secretariat for the Prevention of Money or Asset Laundering (SEPRELAD) - Paraguay',
  'Secretariat of the Interior (SEGOB)',
  'Secretary of the Commonwealth of Massachusetts',
  'Securities and Exchange Board of India',
  'Securities and Exchange Commission - Ghana',
  'Securities and Exchange Commission - Thailand',
  'Securities and Exchange Commission of Nigeria',
  'Securities and Exchange Surveillance Commission',
  'Securities and Futures Commission of Hong Kong',
  'Securities Commission of the Republic of Srpska',
  'Securities Commission Malaysia',
  'Seimas',
  'Sejm',
  'Senado Federal (Federal Senate) - Brazil',
  'Senate Committee on Banking, Housing, and Urban Affairs',
  'Senate of Argentina',
  'Senate of Belgium',
  'Senate of Canada',
  'Senate of Chile',
  'Senate of Colombia',
  'Senate of France',
  'Senate of Mexico',
  'Senate of Netherlands',
  'Senate of Pakistan',
  'Senate of Paraguay',
  'Senate of Poland',
  'Senate of Spain',
  'Senate of Thailand',
  'Senate of the Philippines',
  'Senate of the Republic - Italy',
  'Senegal Data Protection Commission',
  'Senegal National Financial Information Processing Unit',
  'Sepblac (Spanish FIU)',
  'Serbian Commission for Protection of Competition',
  'Serbian Government',
  'Serbian Ministry of Finance',
  'Serbian Ministry of Trade, Tourism, and Telecommunications',
  'Serbian National Assembly',
  'Serbian Regulatory Authority For Electronic Media',
  'Serious Fraud Office',
  'Seychelles Financial Intelligence Unit',
  'Singapore Fintech Association',
  'Singapore Police Force',
  'Sistema de Tarjetas y Medios de Pago',
  'Slovakia Competition Authority',
  'Slovakia National Security Office',
  'Slovenia Central Bank',
  'Slovenia Ministry of Digital Transformation',
  'Slovenian Competition Protection Agency',
  'Slovenian Securities Market Agency',
  'Solicitors Regulation Authority',
  'South African Government',
  'South African National Treasury',
  'South African Parliament',
  'South African Reserve Bank',
  'South Carolina Attorney General',
  'South Carolina Board of Financial Institutions',
  'South Carolina Commissioner of Consumer Finance',
  'South Carolina Department Of Consumer Affairs',
  'South Carolina Governor',
  'South Carolina Legislature',
  'South Dakota Attorney General',
  'South Dakota Department of Labor and Regulation',
  'South Dakota Division of Banking',
  'South Dakota Division of Insurance',
  'South Dakota Governor',
  'South Dakota Legislature',
  'South Korea Fair Trade Commission',
  'South Korea Ministry of Economy and Finance',
  'South Korea National Cyber Security Center',
  'South Korean Financial Intelligence Unit',
  'South Korean Government',
  'Spanish Banking Association',
  'Spanish Chamber of Deputies',
  'Spanish Confederation of Savings Banks',
  'Spanish Congress of Deputies',
  'Spanish Council of Ministers',
  'Spanish Government',
  'Spanish National Securities Market Commission (Comisión Nacional del Mercado de Valores)',
  'Sri Lanka Consumer Affairs Authority',
  'Sri Lanka Data Protection Authority',
  'St. Louis Federal Reserve',
  'State Agency for National Security',
  'State Bank of Pakistan',
  'State Bank of Vietnam',
  'State Commission for the Protection of Economic Competition',
  'State Council of The People\'s Republic of China',
  'State Duma',
  'State Financial Monitoring Service of Ukraine',
  'State Financial Supervision and Administration Bureau',
  'State Investigation and Protection Agency',
  'State of Connecticut Department of Consumer Protection',
  'State Revenue Service',
  'State Secretary for Economic Affairs and Climate',
  'State Securities Commission of Vietnam',
  'State Service of Special Communication and Information Protection of Ukraine',
  'States General of the Netherlands',
  'Stortinget',
  'Superintendence of Companies of Colombia',
  'Superintendencia Financiera de Colombia',
  'Superintendency of Banks of Ecuador',
  'Superintendency of Banks of Panama',
  'Superintendency of Banks of the Dominican Republic',
  'Superintendency of Non-Financial Entities of Panama',
  'Superintendency of the Financial System (SSF)',
  'Superintendency of the Securities Market of Panama',
  'Supreme Audit Office',
  'Supreme Court of India',
  'Sveriges Riksbank',
  'Swedish Authority for Privacy Protection (IMY)',
  'Swedish Bankers\' Association',
  'Swedish Competition Authority',
  'Swedish Consumer Agency',
  'Swedish Consumers\' Association',
  'Swedish Economic Crime Authority',
  'Swedish Fintech Association',
  'Swedish National Debt Office',
  'Swedish Police',
  'Swedish Post and Telecom Authority (PTS)',
  'Swiss Bankers Association',
  'Swiss Competition Commission',
  'Swiss Federal Assembly',
  'Swiss Federal Council',
  'Swiss Financial Market Supervisory Authority',
  'Swiss National Bank',
  'Swiss National Center for Cybersecurity',
  'Swiss Parliament',
  'SwissBanking',
  'TAFEX',
  'Taiwan Central Deposit Insurance Corporation',
  'Taiwan Financial Information Sharing and Analysis Center',
  'Taiwan Financial Supervisory Commission',
  'Taiwan National Treasury Administration',
  'Taiwanese Personal Data Protection Commission',
  'Tajikistan Antimonopoly Service',
  'Tajikistan Communication Service',
  'Tajikistan Ministry of Industry and New Technologies',
  'Tanzania Capital Market and Securities Authority',
  'Tanzanian Government',
  'Tanzanian Parliament',
  'Telecom Regulatory Authority of India',
  'Tennessee Attorney General',
  'Tennessee Department of Commerce and Industry (DCI)',
  'Tennessee Department of Financial Institutions',
  'Tennessee General Assembly',
  'Tennessee Governor',
  'Tennessee Legislature',
  'Texas Attorney General',
  'Texas Department of Banking',
  'Texas Department of Savings and Mortgage Lending',
  'Texas Governor',
  'Texas Legislature',
  'Texas Secretary of State',
  'Texas State Securities Board',
  'Thai Anti-Money Laundering Office',
  'Thai Bankers Association',
  'Thai Committee for the Protection of Credit Information',
  'Thai Council of State',
  'Thai Deposit Protection Agency',
  'Thai Digital Economy Promotion Agency',
  'Thai Digital Government Development Agency',
  'Thai E-commerce Association',
  'Thai Fintech Association',
  'Thai Parliament',
  'Thai Revenue Department',
  'Thailand Computer Emergency Response Team',
  'Thailand Supervision and Inspection Division AMLO Office',
  'Thailand Trade Competition Commission',
  'The Association of Banks In Cambodia',
  'The Association of Banks in Malaysia',
  'The Berlin Group',
  'The Clearing House',
  'The Financial Crimes Investigation Service (FNTT)',
  'The Fiscal Nacional Económica - Chile',
  'The Head of the Cabinet of Ministers of Argentina',
  'The Law Society',
  'The Office of the Information and Data Protection Commissioner',
  'The Payment and Electronic Money Institutions Association of Türkiye',
  'The State Council of the Republic of Korea',
  'The State House Abuja',
  'The Superintendence of Industry and Commerce (SIC) - Colombia',
  'The UK Cards Association',
  'The White House',
  'TRACFIN',
  'Transparency International UK',
  'Treasury Board of Canada',
  'Treasury Committee',
  'Treasury of Australia',
  'Turkey Information and Communication Technologies Authority',
  'Turkish Association of Payment and Electronic Money Institutions',
  'Turkish Banking Regulation and Supervision Agency',
  'Turkish Personal Data Protection Authority',
  'Turkmenistan Ministry of Communications',
  'Turkmenistan Ministry of Trade and Foreign Economic Relations',
  'UAE - Securities and Commodities Authority (SCA)',
  'UAE Banks Federation',
  'UAE Cabinet',
  'UAE Consumer Protection Department',
  'UAE Data Office',
  'UAE Executive Office of Anti-Money Laundering and Counter Terrorism Financing',
  'UAE Federal National Council',
  'UAE Financial Intelligence Unit',
  'UAE Ministry of Economy',
  'UAE Ministry of Justice',
  'UAE Securities and Commodities Authority',
  'Uganda Personal Data Protection Office',
  'UK Competition and Markets Authority',
  'UK Department for Business, Energy and Industrial Strategy',
  'UK Department for Digital, Culture, Media & Sport',
  'UK Department for Work and Pensions',
  'UK Finance',
  'UK Financial Reporting Council',
  'UK Gambling Commission',
  'UK Government',
  'UK Parliament',
  'UK Pensions Regulator',
  'UK Supreme Court',
  'Undersecretary of State for the Economy',
  'Uniform Law Commission',
  'Unique Identification Authority of India',
  'Unità di Informazione Finanziaria',
  'United States Virgin Islands Department of Justice',
  'Uruguay Government',
  'Uruguay National Secretariat for the Fight against Money Laundering and the Financing of Terrorism',
  'Uruguay Parliament',
  'Uruguay Superintendency of Financial Services',
  'US Bank Policy Institute',
  'US Congress',
  'US Department of Justice',
  'US Department of the Treasury',
  'US Federal Housing Finance Agency',
  'US House Committee on Financial Services',
  'US Payments Forum',
  'US Securities and Exchange Commission',
  'US Supreme Court',
  'US Virgin Islands Department of Justice',
  'US Virgin Islands Division of Banking, Insurance & Financial Regulation',
  'US Virgin Islands Lieutenant Governor',
  'Utah Attorney General',
  'Utah Department of Commerce',
  'Utah Department of Financial Institutions',
  'Utah State Legislature',
  'Uzbekistan Ministry for Development of Information Technologies and Communications',
  'Vanuatu Financial Intelligence Unit',
  'Vanuatu Financial Services Commission',
  'Variable Recurring Payments (Non-Sweeping) Working Group',
  'Venezuelan Government',
  'Venezuelan Superintendency of Banking Sector Institutions',
  'Verein für Konsumenteninformation',
  'Verkhovna Rada of Ukraine',
  'Vermont Agency of Digital Services',
  'Vermont Attorney General',
  'Vermont Department of Financial Regulation (DFR)',
  'Vermont General Assembly',
  'Vermont Governor',
  'Vermont Legislature',
  'Vietnam Ministry of Information and Communications',
  'Vietnam National Assembly',
  'Vietnamese Banks Association',
  'Vietnamese Government',
  'Virginia State Corporation Commission',
  'Virginia Attorney General',
  'Virginia Bureau of Financial Institutions',
  'Virginia General Assembly',
  'Virginia Governor',
  'Virginia IT Agency',
  'Virginia Legislature',
  'Wall Street Blockchain Alliance',
  'Washington State Department of Financial Institutions',
  'Washington State Legislature',
  'West Virginia Legislature',
  'Wisconsin Attorney General',
  'Wisconsin Department of Financial Institutions (DFI)',
  'Wisconsin State Legislature',
  'Wolfsberg Group',
  'World Bank Group',
  'Wyoming State Legislature',
];

export const KNOWN_AUTHORITIES_SET = new Set(KNOWN_AUTHORITIES.map(a => a.toLowerCase()));

export function canonicalAuthority(name) {
  if (!name) return null;
  const lower = name.toLowerCase().trim();
  return KNOWN_AUTHORITIES.find(a => a.toLowerCase() === lower) || null;
}

...
// Skip index/contents navigation hrefs — we want actual document links only
const SKIP_HREF_RE = /\/contents(?:[/?#]|$)/i;
...
export async function scrapeSource(source) {
  ...
  for (const { href, text: rawTitle } of anchors) {
    ...
    if (source.hrefFilter && !source.hrefFilter.test(href)) continue;
